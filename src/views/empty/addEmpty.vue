  ...
  <div>
    <div v-if="step==2">
      ...
      <header class="myheader">
        <button data-v-5a2146fc="" class="mint-button mint-button--default mint-button--normal" @click="step=1"><span class="mint-button-icon"><i class="mintui mintui-back"></i></span> </button>
      ...
      <footer class="clearfix">
        ...
        <div class="footer-tag-group flexable" v-show="tagGroupShow">
          <span class="selected-tag" v-for="(tag,index) in selectItem" :key="index" v-if="tag.type!=1">{{tag.nameAlias}}
        ...
        <div class="footer-bottom">
          ...
          <span style="display: inline-block;float: right;">
            <span style="display: inline-block;">
              <p class="footer-total">共计{{ this.selectItem.length-oldLength}}人 </p>
    ...
    <div v-if="step==1" style="padding-bottom: 2.2rem">
      <mt-header title="修改特殊考勤人员" fixed>
        <div slot="left">
      ...
      <footer class="flexable empty-footer myfooter">
        ...
        <div class="left-btn flexable" @click="addNext">继续添加</div>
      </footer>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getScheEmptyUpd() {
      ...
      this.$api
        .getScheEmptyUpd(params)
        .then(response => {
          console.log(response)
          if (response.data.respCode == 0) {
            this.selectItem = response.data.data
            console.log(this.selectItem)
            this.oldLength = this.selectItem.length
            this.selectItem = this.selectItem.map(function (obj, index) {
              //                  obj.name=obj.talentName
              obj.type = 1
              obj.nameAlias = obj.talentName
              obj.endWorkType = obj.schedulingPunchList[0].customerEndWorkType
              obj.workEndTimeShow = (obj.schedulingPunchList[0].schedulingDate == obj.schedulingPunchList[0].workEndDate ? '当日' : '次日') + obj.schedulingPunchList[0].workEndTime.substring(0, 5)
              obj.workEndTime = obj.schedulingPunchList[0].workEndTime.substring(0, 5)
              obj.workStartTime = obj.schedulingPunchList[0].workStartTime.substring(0, 5)
              console.log(obj)
              return obj;
            })
          }
    ...
    handleSubmit() {
      ...
      for (let x in this.selectItem) {
        ...
        if (!obj.workEndTime) {
          that.$toast(`请选择${obj.nameAlias}下班时间`)
          valid = false
          return false;
        } else if (!obj.workStartTime) {
          that.$toast(`请选择${obj.nameAlias}上班时间`)
          valid = false
          return false;
        } else if (!obj.industryName) {
          that.$toast(`请选择${obj.nameAlias}工种`)
          valid = false
          return false;
        }
      }
      ...
      for (let x in this.selectItem) {
        ...
        newobj.sex = obj.sex
        let id;
        if (obj.schedulingPunchList) {
          id = obj.schedulingPunchList[0].id
        }
    ...
    RouteToNext() {
      if (this.selectItem.length - this.oldLength > Number(this.mostNumber)) {
