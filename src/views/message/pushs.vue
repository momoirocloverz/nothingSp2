<template>
    <div class="news-3011">
        <div class="text tc pt15" v-if="show">暂无业务推送消息</div>
        <div :style="{'-webkit-overflow-scrolling': scrollMode}" v-if="!show" class="listCon">
            <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded"
                        :auto-fill="false" ref="pushLoadmore" @top-status-change="handleTopChange" @bottom-status-change="handleBottomChange">
                <ul>
    ...
    export default {
        ...
        methods: {        
            ...
            loadTop(){
                ...
                this.getList();
                this.$refs.pushLoadmore.onTopLoaded();
            },
            loadBottom(){
                this.currentPage++
                this.getListMore()
                this.$refs.pushLoadmore.onBottomLoaded();
            },
            goDetail(row) {
                this.$router.push({
                    name: 'messageDetails',
                    query: Object.assign({title:'业务推送',type:'2'},row)
                })
            },
            getList() {
